...
use etherdream::{ client::{ self, Client }, DeviceInfo, generator };
use etherdream_simulator::{ self as simulator, Simulator };

#[tokio::test]
async fn a_generator_will_publish_point_data() {
  let capacity = 10;
  let ( mut simulator, client ) = setup_with_capacity( capacity ).await;
  ...
  assert_eq!( wait_for_simulator_invocation( &mut invoked_rx ).await, capacity );
  assert_eq!( wait_for_simulator_invocation( &mut invoked_rx ).await, capacity );

  // Validate that the simulator is at capacity
  assert!( wait_for_simulator_count( &simulator, capacity ).await );
...
async fn a_generator_can_ping_the_device() {
  let ( _, client ) = setup_with_capacity( 10 ).await;
...
// Creates an Etherdream simulator and client.
async fn setup_with_capacity( capacity: usize ) -> ( Simulator, Client ) {
